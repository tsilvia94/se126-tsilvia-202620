#W4 - Parellel List Review, Sequential Search, and Creating + Writing to New Text Files

#PROGRAM PROMPT: We will work with the class_grades.csv file. We will practice connecting to a file, storing the file data into parallel lists, and creating new data for each student record based on these lists. We will then build a sequential search program which will allow us to find students in the file, and write data regarding them to a newly created file in our repository.
...
file = open("demos/w4/compiled_class_info.csv", "w") #creates file to write o
        ...
        for i in range(0, len(lastName)):
            ...
            if search_last.lower() == lastName[i].lower():
                ...
                found = i #replace the invalid found value with the current index
                          #this current index reps the LOCATION of the found, searched-for, last name
                          #if we know the location (index) of the last name ... we also know the location of all associated data! (bc they share index)
